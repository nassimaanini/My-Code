...
class Show {
     ...
     */
    private function fronteEnd($bill_invoiceid) {
        //merge data
        request()->merge([
        ...
        $bondelivraison = \App\Models\Invoice::Where([["bill_type","bondelivraison"],["related_invoice_id",$bill_invoiceid]])->first();
        if (!$bondelivraison) {
            config([
                'visibility.edit_invoice' => true,
            ]);
        }
        $avoir = \App\Models\Invoice::Where([["bill_type","avoir"],["related_invoice_id",$bill_invoiceid]])->first();
        if (!$avoir) {
            config([
                'visibility.convert_invoice' => true,
            ]);
        }
