...
use App\Export\ClientsExportCSV;
...
class Clients extends Controller
{
    ...
    public function index(CategoryRepository $categoryrepo)
    {
        ...
        //client categories
        $categories = $categoryrepo->get('secteurs');
    ...
    public function store(ClientStoreValidation $request)
    {
        ...
        if(!is_numeric(request('client_company_ice')) && request('client_company_ice') != ''){
            abort(409,__('lang.ice')." ".__('lang.must_only_contain_numbers'));
        }
    ...
    public function showDynamic($id)
    {
        ...
        switch (request()->segment(3)) {
            ...
            case 'avoirs':
                $sections = request()->segment(3);                
                $page['dynamic_url'] = url($sections . '?source=ext&invoiceresource_type=client&invoiceresource_id=' . $client->client_id);
                break;
            case 'bondelivraisons':
                $sections = request()->segment(3);                
                $page['dynamic_url'] = url($sections . '?source=ext&invoiceresource_type=client&invoiceresource_id=' . $client->client_id);
            ...
            case 'projects':            
                ...
                $section = rtrim($sections, 's');
                $page['dynamic_url'] = url($sections . '?source=ext&' . $section . 'resource_type=client&' . $section . 'resource_id=' . $client->client_id);
                // $page['dynamic_archived_url'] = url($sections . '?source=ext&' . $section . 'resource_type=client&' . $section . 'resource_id=' . $client->client_id);
                break;
            case 'project-files':
                $sections = request()->segment(3);
            ...
            case 'files':
                ...
                $page['dynamic_url'] = url($sections . '?source=ext&' .  'fileresource_type=client&' .  'fileresource_id=' . $client->client_id);
                break;
            case 'details':
    ...
    public function destroy(DestroyRepository $destroyrepo)
    {
        ...
        $payload = [
            ...
            'count' => $count,
            'page' => [
                'no_results_message' => __('lang.no_results_found_clients'),
            'no_results_sub_message'=>__('lang.no_results_sub_clients'),
            ],
        ];
    ...
    public function Exportxsl(){
        ...
        $client_export = new ClientsExport($allrows);
        $file_name = "Export_clients_" . time() . ".xlsx";
        $client_export->store($file_name, 'local'); 

        $payload = ['download_export_url' =>  url('/storage/' . $file_name),'file_name' => $file_name];
    ...
    public function Exportcsv(){
        ...
        $client_export = new ClientsExportCSV($allrows);
        $file_name = "Export_clients_" . time() . ".csv";
        $client_export->store($file_name, 'local'); 
    ...
    public function updateDescription()
    {
        ...
        $jsondata['redirect_url'] = url("/clients/" . $client->client_id . '/details');
