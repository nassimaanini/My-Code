...
class UpdateResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('ref') == 'list') {
            if(request("invoiceresource_type") == "client" && request("invoiceresource_id")){
                $jsondata['redirect_url'] = url('clients/'.request("invoiceresource_id").'/invoices');
            }elseif(request("invoiceresource_type") == "project" && request("invoiceresource_id")){
                $jsondata['redirect_url'] = url('projects/'.request("invoiceresource_id").'/invoices');
            }
            else{
                //replace the row of this record
                $html = view('pages/invoices/components/table/ajax', compact('invoices'))->render();
                $html = $html . "<p> azertyh </p> ";
